...
A=dir('FFTIFs/*t001_z001_c001.tif');

for a=1:length(A) 
    
    Exp(a)=regexp(A(a).name,'(\w+)t001_z001_c001.tif','tokens'); 
...
X=zeros(size(A));
for a = 1:(length(A))
    N=(regexp(A(a).name,'\w+t(\d+)_z\w+','tokens'));
    N=cell2mat(N{1});
    X(a)=str2double(N);
end
idx=unique(X);
...
for a = 1:TotalPos
    mkdir(strcat(OutputFolder,'/Pos',num2str(Count)));
       ...
       A = dir (strcat(InputFolder,'/',Experiments{jj},'t*',sprintf('%03d',idx(a)),'_z*c',sprintf('%03d',1),'*'));
       
        for c=1:length(A)
            
            
            Img = double(imread(strcat(InputFolder,'/',Experiments{jj},'t',sprintf('%03d',idx(a)),'_z',sprintf('%03d',c),'_c001.tif')));
            ...
            Img2 = double(imread(strcat(InputFolder,'/',Experiments{jj},'t',sprintf('%03d',idx(a)),'_z',sprintf('%03d',c),'_c002.tif')));
